...
class AccountCaReport(models.Model):
    _name = "account.ca.report"
    _description = "Turn over analysis"
    _auto = False
    _rec_name = 'date'
    ...
    origin_id = fields.Many2one('sale.order', string='Commande', readonly=True)
    flag = fields.Selection(string="type de commande",
                             selection=[
                                 ('repair', 'REPARATION'),
                                 ('quotation', 'NEGOCE'),
                             ],
    ...
    def _select(self):
        select_str = """
            SELECT sub.id, sub.date, sub.product_id, sub.partner_id, sub.country_id, sub.account_analytic_id,
                ...
                COALESCE(cr.rate, 1) as currency_rate, sub.residual as residual, sub.commercial_partner_id as commercial_partner_id, sub.number, sub.origin_id, sub.flag
        """
        return select_str
    ...
    def _sub_select(self):
        select_str = """
                SELECT ail.id AS id,
                    ...
                    partner.country_id, ai.number as number, ai.origin_id as origin_id, (SELECT flag FROM sale_order WHERE id = ai.origin_id) as flag
        """
        return select_str
