...
class StockSchedulerCompute(models.TransientModel):
    ...
    def _procure_calculation_orderpoint(self, partner_id=False):
        with api.Environment.manage():
            ...
            for company in self.env.user.company_ids:
                self.env['procurement.group'].run_scheduler(
                    ...
                    company_id=company.id, partner_id=partner_id)
            
            new_cr.close()
            return {}

    def procure_calculation(self):
        threaded_calculation = threading.Thread(target=self._procure_calculation_orderpoint, args=(self.partner_id))
